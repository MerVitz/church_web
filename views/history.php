...
<main class="flex-grow bg-[#ffffff] py-8 px-4">
    <h2 class="text-4xl font-bold text-[#6C8BC9] text-center mb-6">History of the Anglican Church of Kenya</h2>
            ...
            <?php foreach ($history as $row): ?>
                <div class="flex flex-col md:flex-row <?php echo ($row['alignment'] == 'right') ? 'md:flex-row-reverse' : ''; ?> items-center md:items-start">
                    <div class="md:w-1/2 <?php echo ($row['alignment'] == 'left') ? 'text-right md:pr-10' : 'text-left md:pl-10'; ?> animate-fadeIn">
                        <h3 class="text-2xl font-semibold text-[#6C8BC9]"><?= htmlspecialchars($row['year']); ?> - <?= htmlspecialchars($row['title']); ?></h3>
                        <p class="text-black mt-2"><?= nl2br(htmlspecialchars($row['description'])); ?></p>
                    </div>
                    <div class="w-24 h-24 bg-[#ffffff] rounded-full overflow-hidden border-4 border-[#6C8BC9] shadow-lg">
